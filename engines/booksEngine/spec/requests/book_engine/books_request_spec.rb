...
module BookEngine
  RSpec.describe "Books", type: :request do
    context 'GET /book_engine/api/v1/books' do
      context 'Without books' do
        it 'Gets an empty array' do
          get '/book_engine/api/v1/books'
    ...
    context 'GET /book_engine/api/v1/books/search' do
      ...
      context 'With stubbed api request' do
        it 'Searches books' do
          ...
          get '/book_engine/api/v1/books/search?q=cleancode'
          expect(response.status).to eq(200)
      ...
      context 'Without a query parameter specified' do
        it 'returns 400' do
          get '/book_engine/api/v1/books/search'
    ...
    context 'POST /book_engine/api/v1/books' do
      let(:book_attrs) do
        {
      ...
      it 'Creates a book' do
        expect {
          post '/book_engine/api/v1/books', params: {book: book_attrs, publisher: publisher_attrs}
      ...
      context 'With existing publisher' do
        it 'Does not create a publisher' do
          expect {
            post '/book_engine/api/v1/books', params: {book: book_attrs, publisher: {name: publisher.name}}
      ...
      context 'With new categories specified in params' do
        ...
        it 'Creates categories' do
          expect {
            post '/book_engine/api/v1/books', params: {book: book_attrs, publisher: publisher_attrs, categories: categories_attrs}
    ...
    context 'GET /book_engine/api/v1/books:id' do
      let!(:book) { create(:book) }

      it 'Gets a book' do
        get "/book_engine/api/v1/books/#{book.id}"
        expect(response.status).to eq(200)
    ...
    context 'PUT /book_engine/api/v1/books:id' do
      let!(:book) { create(:book) }
      let(:new_title) { Faker::Lorem.characters(number: 20) }

      it 'Updates a book' do
        put "/book_engine/api/v1/books/#{book.id}", params: {book: {title: new_title}}
        expect(response.status).to eq(200)
        expect(JSON.parse(response.body)['title']).to eq new_title
      end
    end

    context 'DELETE /book_engine/api/v1/books:id' do
      let!(:book) { create(:book) }

      it 'Deletes a book' do
        expect {
          delete "/book_engine/api/v1/books/#{book.id}"
        }.to change { Book.count }.by(-1)
